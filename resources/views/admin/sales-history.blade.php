...
@section('content')
    ...
    <div class="flex-1 p-6">
      ...
      <div class="mb-6 flex flex-col md:flex-row gap-4">
        <div class="relative flex-1">
          <input type="text" id="salesSearch" x-model="searchTerm" placeholder="Search by customer or sale ID..." class="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
        ...
        <div class="flex flex-col gap-2">
          <div class="flex space-x-2">
            <button :class="{'bg-green-600 text-white': filterType === '7days', 'bg-gray-200 text-gray-700': filterType !== '7days'}" @click="setFilter('7days')" class="px-4 py-2 rounded-lg hover:bg-green-700 hover:text-white">Last 7 Days</button>
            <button :class="{'bg-green-600 text-white': filterType === '30days', 'bg-gray-200 text-gray-700': filterType !== '30days'}" @click="setFilter('30days')" class="px-4 py-2 rounded-lg hover:bg-green-700 hover:text-white">Last 30 Days</button>
            <button :class="{'bg-green-600 text-white': filterType === 'custom', 'bg-gray-200 text-gray-700': filterType !== 'custom'}" @click="setFilter('custom')" class="px-4 py-2 rounded-lg hover:bg-green-700 hover:text-white">Custom Range</button>
          </div>
          <div x-show="filterType === 'custom'" class="flex space-x-2">
            <input type="date" x-model="customStartDate" class="p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
            <input type="date" x-model="customEndDate" class="p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
            <button @click="applyCustomFilter" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700">Apply</button>
          </div>
        </div>
      </div>
            ...
            <template x-for="sale in filteredSales" :key="sale.id">
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                  <button class="view-details text-green-600 hover:text-green-800" :data-sale-id="sale.id">View Details</button>
                </td>
              </tr>
            </template>
          </tbody>
  ...
  <!-- JavaScript for Modal and Alpine.js Logic -->
  <script>
      ...
      Alpine.data('salesFilter', () => ({
        ...
        get filteredSales() {
          ...
          if (this.filterType === '7days') {
            ...
            return this.saleData; // Show all if no valid custom range
          }
        ...
        get totalRevenue() {
          const total = this.filteredSales.reduce((sum, sale) => {
            const amount = parseFloat(sale.totalAmount.replace('₦', '').replace(/,/g, ''));
            return sum + amount;
          }, 0);
        ...
        get revenueDescription() {
          ...
          if (this.customStartDate && this.customEndDate) {
            return `Revenue from sales between ${this.customStartDate} - ${this.customEndDate}`;
          }
    ...
    // Modal handling (plain JavaScript for consistency with previous version)
